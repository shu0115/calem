class SchedulesController < ApplicationController
  ...
  def index(now_date)
    ...
    @schedules = Schedule.where(user_id: current_user.id).where(start_time: (@now_date.beginning_of_month.beginning_of_day..@now_date.end_of_month.end_of_day)).order("start_time ASC").all
    @schedules.each{ |s|
